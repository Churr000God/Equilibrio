...
class GoalDaoTest {
    ...
    private suspend fun contribute(goalId: String, id: String, cents: Long, at: Long = 0, deleted: Boolean = false) {
        database.transactionDao().upsert(
            TransactionEntity(
                id = "tx-$id", userId = userId, accountId = accountId, kind = "EXPENSE", classification = null,
                amountCents = cents, occurredAt = at, note = null, categoryKey = "savings", updatedAt = 0, syncState = "PENDING",
            ),
        )
        database.goalDao().insertContribution(
            GoalContributionEntity(
                id = id, goalId = goalId, transactionId = "tx-$id", amountCents = cents, occurredAt = at,
    ...
    @Test
    fun savedCentsIsSumOfLiveContributions() = runBlocking {
        database.goalDao().upsert(goal("g1"))
        contribute("g1", "c1", 200_00)
        contribute("g1", "c2", 20_00)
        contribute("g1", "c3", 999_00, deleted = true)

        val fetched = database.goalDao().getById("g1")!!
        assertEquals(220_00L, fetched.savedCents)
    }
    ...
    @Test
    fun deletingByTransactionRemovesContribution() = runBlocking {
        database.goalDao().upsert(goal("g1"))
        contribute("g1", "c1", 100_00)
        database.goalDao().markContributionDeletedByTransaction("tx-c1", now = 1)

        assertEquals(0L, database.goalDao().getById("g1")!!.savedCents)
    }

    @Test
    fun deletingGoalRemovesItsContributionDates() = runBlocking {
        database.goalDao().upsert(goal("g1"))
        contribute("g1", "c1", 100_00, at = 1000)
        database.goalDao().markDeleted("g1", now = 1)
        database.goalDao().markContributionsDeletedByGoal("g1", now = 1)
