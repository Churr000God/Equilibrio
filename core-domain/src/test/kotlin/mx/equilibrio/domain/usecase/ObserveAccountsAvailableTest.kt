...
class ObserveAccountsAvailableTest {
    ...
    fun `cada tipo usa su propio calculo de disponible`() = runTest {
        ...
        val byId = useCase().first().associate { it.account.id to it.availableCents }
    ...
    @Test
    fun `tarjeta sin periodo activo cae a su balance guardado`() = runTest {
        val byId = useCase().first().associate { it.account.id to it.availableCents }

        assertEquals(card.balanceCents, byId["cc"])
    }
